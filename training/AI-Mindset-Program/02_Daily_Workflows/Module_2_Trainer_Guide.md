...
### Slides 10–11 (Cursor)
For mixed audiences, frame codebase Q&A as valuable even for non-coders who work *near* code (BAs, PMs): "the end of 'wait for a developer to explain it.'" The leash rule (slide 11) is the module's most important safety rail for engineers: inline edit = review each hunk; agent mode = review the whole diff like a hostile PR. The Project Velocity sidebar earns you the right to say this works at enterprise scale — 20 apps, 3 weeks, 187 endpoints — but attribute honestly: that result came from a designed pipeline with verification gates (Module 4), not from casual chat with an IDE.
